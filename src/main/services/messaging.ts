...
import { forkActionHook } from '~/main/hook';
...
export const runMessagingService = (appWindow: AppWindow) => {
  ...
  if (process.env.ENABLE_AUTOFILL) {
    ...
    ipcMain.on(`credentials-save-${id}`, async (e, data) => {
      ...
      await setPassword('point', `${hostname}-${username}`, password);
    ...
    ipcMain.on(`credentials-remove-${id}`, async (e, data: IFormFillData) => {
      ...
      await deletePassword('point', `${view.hostname}-${fields.username}`);
    ...
    ipcMain.on(
      ...
      async (e, id: string, account: string) => {
        const password = await getPassword('point', account);
  ...
  if (process.env.FORK) {
    forkActionHook('messaging', 'load', id, appWindow);
  }
};
